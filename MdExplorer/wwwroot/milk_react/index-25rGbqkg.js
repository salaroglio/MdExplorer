import { i as P, d as T, P as nt, w as st, b as it } from "./index.es-Dganzb1x.js";
import { P as ot, n as j, o as v, T as at, q as xe, c as Ae, p as lt, t as c, r as E, u as ht, v as ft, w as ut, h as _, a as ae, x as dt, f as Be, i as pt, y as ct, C as mt, L as le, z as gt } from "./index-B3KiKTSt-Dla4yOC_.js";
import { html as kt, htmlCompletionSource as Lt } from "./index-BXEC2rDE.js";
class $ {
  static create(e, r, n, s, i) {
    let o = s + (s << 8) + e + (r << 4) | 0;
    return new $(e, r, n, o, i, [], []);
  }
...
`);
  ...
  */
  startContext(e, r, n = 0) {
    this.block = $.create(e, n, this.lineStart + r, this.block.hash, this.lineStart + this.line.text.length), this.stack.push(this.block);
  }
  /**
    ...
    if (y(r.defineNodes)) {
      ...
      for (let L of r.defineNodes) {
        ...
        d.push(j.define({
          ...
          name: k,
          props: M && [[v.group, M]]
        })), b && (m || (m = {}), Array.isArray(b) || b instanceof at ? m[k] = b : Object.assign(m, b));
      }
      n = new xe(d), m && (n = n.extend(Ae(m)));
    }
    ...
    if (y(r.parseBlock))
      for (let d of r.parseBlock) {
        ...
        if (m > -1)
          ...
          let L = d.before ? F(a, d.before) : d.after ? F(a, d.after) + 1 : a.length - 1;
    ...
    if (y(r.parseInline))
      for (let d of r.parseInline) {
        ...
        if (m > -1)
          ...
          let L = d.before ? F(f, d.before) : d.after ? F(f, d.after) + 1 : f.length - 1;
...
const Fe = { resolve: "Emphasis", mark: "EmphasisMark" }, $e = { resolve: "Emphasis", mark: "EmphasisMark" }, H = {}, W = {};
class C {
  constructor(e, r, n, s) {
...
const Q = {
  ...
  Emphasis(t, e, r) {
    ...
    return t.append(new C(e == 95 ? Fe : $e, r, n, (d ? 1 : 0) | (m ? 2 : 0)));
  },
  HardBreak(t, e, r) {
  ...
  resolveMarkers(e) {
    for (let n = e; n < this.parts.length; n++) {
      ...
      if (!(s instanceof C && s.type.resolve && s.side & 2))
        continue;
      let i = s.type == Fe || s.type == $e, o = s.to - s.from, a, l = n - 1;
      for (; l >= e; l--) {
        let k = this.parts[l];
...
const Pt = Ae({
  ...
  LinkTitle: c.string,
  Paragraph: c.content
}), vt = new q(new xe(ze).extend(Pt), Object.keys(X).map((t) => X[t]), Object.keys(X).map((t) => Re[t]), Object.keys(X), xt, Ie, Object.keys(Q).map((t) => Q[t]), Object.keys(Q), []);
function Nt(t, e, r) {
  let n = [];
...
}, me = /(www\.)|(https?:\/\/)|([\w.+-]{1,100}@)|(mailto:|xmpp:)/gy, ge = /[\w-]+(\.[\w-]+)+(\/[^\s<]*)?/gy, Xt = /[\w-]+\.[\w-]+($|\/)/, ke = /[\w.+-]+@[\w-]+(\.[\w.-]+)+/gy, Le = /\/[a-zA-Z\d@.]+/gy;
function be(t, e, r, n) {
  let s = 0;
  for (let i = e; i < r; i++)
    t[i] == n && s++;
  ...
  parseInline: [{
    ...
    parse(t, e, r) {
      ...
      if (!s)
        return -1;
      if (s[1] || s[2]) {
        if (i = Ft(t.text, n + s[0].length), i > -1 && t.hasOpenLink) {
...
}, Ge = /* @__PURE__ */ ft({ commentTokens: { block: { open: "<!--", close: "-->" } } }), Ve = /* @__PURE__ */ new v(), Ke = /* @__PURE__ */ vt.configure({
  props: [
...
function ie(t) {
  return new ht(Ge, t, [Gt], "markdown");
}
...
const Yt = ({ state: t, dispatch: e }) => {
  let r = _(t), { doc: n } = t, s = null, i = t.changeByRange((o) => {
    if (!o.empty || !Je.isActiveAt(t, o.from, 0))
      return s = { range: o };
...
function Ce(t, e, r) {
  ...
  for (let s = 0, i = t.length - 2; s <= i; s++)
    n += t[s].blank(s < i ? T(r.text, 4, Math.min(r.text.length, t[s + 1].from)) - n.length : null, s < i);
  return oe(n, e);
}
function er(t, e) {
...
function lr(t = {}) {
  ...
  a.push(yt({ codeParser: h, htmlParser: o.language.parser })), n && l.push(nt.high(st.of(rr)));
...
function sr() {
  if (V)
    return V;
  let t = Lt(new mt(it.create({ extensions: et }), 0, !0));
