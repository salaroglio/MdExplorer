document.write("<script src='/bootstrap/jquery-3.6.0.js'></script>");
document.write("<script src='/bootstrap/js/bootstrap.bundle.js'></script>");
document.write("<script src='/jquery_ui/jquery-ui.js'></script>");
document.write("<script src='/bootstrap_datepicker/js/bootstrap-datepicker.js'></script>");
document.write("<script src='/highlightjs/highlight.min.js'></script>");
document.write("<link href='/prismjs/prism-tomorrow.min.css' rel='stylesheet' />");
document.write("<script src='/prismjs/prism.min.js'></script>");
document.write("<script src='/prismjs/prism-java.min.js'></script>");
document.write("<script src='/prismjs/prism-csharp.min.js'></script>");
document.write("<script src='/prismjs/prism-javascript.min.js'></script>");
document.write("<script src='/prismjs/prism-python.min.js'></script>");
document.write("<script src='/prismjs/prism-sql.min.js'></script>");
document.write("<script src='/javascripts/jqueryForFirstPage.js'></script>");
document.write("<script src='/TocBot/tocbot.min.js'></script>");
document.write("<script src='/jspreadsheet_ce/jsuites.js'></script>");
document.write("<script src='/jspreadsheet_ce/jexcel.js'></script>");
document.write("<script src='/tippy/popper.js'></script>");
document.write("<script src='/tippy/tippy.js'></script>");
document.write("<script src='/highlight_within_textarea/highlight-within-textarea.js'></script>")
                             





