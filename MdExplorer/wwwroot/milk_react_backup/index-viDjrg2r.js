import { L as G, E as R, C as u } from "./index-Crfuao6v.js";
import { c as w, t as $, b as v, a as V, f as g, i as U, l as b, d as q } from "./index-B3KiKTSt-Dla4yOC_.js";
import { defineCSSCompletionSource as E } from "./index-Bjb4cgZN.js";
const z = 154, X = 155, C = 156, N = 1, D = 2, f = 3, I = 157, F = 158, Z = 4, K = 5, L = 159, p = 160, W = 161, s = 162, T = 6, h = 7, A = 8, B = 9, c = 0, i = [
  9,
  10,
  11,
  12,
  ...
  8287,
  12288
], M = 58, J = 40, P = 95, H = 91, l = 45, OO = 46, m = 35, $O = 37, j = 123, eO = 125, o = 47, S = 42, r = 10, _ = 61, aO = 43, QO = 38;
function d(O) {
...
const tO = new R((O, e) => {
  if (e.dialectEnabled(c)) {
    let a;
    if (O.next < 0 && e.canShift(W))
      O.acceptToken(W);
    else if (((a = O.peek(-1)) == r || a < 0) && e.canShift(p)) {
      let Q = 0;
      for (; O.next != r && i.includes(O.next); )
        O.advance(), Q++;
      O.next == r || k(O) ? O.acceptToken(p, -Q) : Q && O.acceptToken(s);
    } else if (O.next == r)
      O.acceptToken(L, 1);
    else if (i.includes(O.next)) {
      for (O.advance(); O.next != r && i.includes(O.next); ) O.advance();
      O.acceptToken(s);
    }
  } else {
    let a = 0;
    for (; i.includes(O.next); )
      O.advance(), a++;
    a && O.acceptToken(s);
  }
}, { contextual: !0 }), nO = new R((O, e) => {
  if (k(O)) {
  ...
  (O.next == aO || O.next == _) && e.dialectEnabled(c) && O.acceptToken(O.next == _ ? A : B, 1);
}), iO = new R((O, e) => {
  ...
  if (O.peek(-1) == r) {
    ...
    for (; O.next != r && i.includes(O.next); )
      O.advance(), t++;
    t != a && O.next != r && !k(O) && (t < a ? O.acceptToken(X, -t) : O.acceptToken(z));
  }
}), RO = new R((O, e) => {
  for (let a = !1, Q = 0, t = 0; ; t++) {
    let { next: n } = O;
    if (d(n) || n == l || n == P || a && x(n))
      !a && (n != l || t > 0) && (a = !0), Q === t && n == l && Q++, O.advance();
    else if (n == m && O.peek(1) == j) {
      O.acceptToken(K, 2);
      break;
    } else {
      a && O.acceptToken(n == J ? I : Q == 2 && e.canShift(Z) ? Z : F);
      break;
    }
  }
}), lO = new R((O) => {
  if (O.next == eO) {
    for (O.advance(); d(O.next) || O.next == l || O.next == P || x(O.next); )
      O.advance();
    O.next == m && O.peek(1) == j ? O.acceptToken(D, 2) : O.acceptToken(N);
  }
}), oO = new R((O) => {
  if (i.includes(O.peek(-1))) {
    let { next: e } = O;
    (d(e) || e == P || e == m || e == OO || e == H || e == M || e == l || e == QO) && O.acceptToken(C);
  }
}), dO = new R((O) => {
  if (!i.includes(O.peek(-1))) {
    let { next: e } = O;
...
const SO = new Y(null, 0), cO = new u({
  start: SO,
  shift(O, e, a, Q) {
    return e == z ? new Y(O, a.pos - Q.pos) : e == X ? O.parent : O;
  },
  hash(O) {
    return O.hash;
  }
  ...
  "[ ]": $.squareBracket,
  "{ }": $.brace
}), XO = { __proto__: null, not: 62, only: 62, using: 179, as: 189, with: 193, without: 193, hide: 207, show: 207, from: 230, to: 232, if: 245, through: 251, in: 257 }, yO = { __proto__: null, url: 80, "url-prefix": 80, domain: 80, regexp: 80, lang: 94, "nth-child": 94, "nth-last-child": 94, "nth-of-type": 94, "nth-last-of-type": 94, dir: 94, "host-context": 94, selector: 172 }, PO = { __proto__: null, "@import": 156, "@include": 176, "@mixin": 182, "@function": 182, "@use": 186, "@extend": 196, "@at-root": 200, "@forward": 204, "@media": 210, "@charset": 214, "@namespace": 218, "@keyframes": 224, "@supports": 236, "@if": 240, "@else": 242, "@for": 248, "@each": 254, "@while": 260, "@debug": 264, "@warn": 264, "@error": 264, "@return": 264 }, mO = G.deserialize({
  version: 14,
  ...
  tokenizers: [iO, oO, lO, dO, RO, tO, nO, rO, 0, 1, 2, 3],
  topRules: { StyleSheet: [0, 10], Styles: [1, 134] },
  dialects: { indented: 0 },
  specialized: [{ term: 158, get: (O) => XO[O] || -1 }, { term: 157, get: (O) => yO[O] || -1 }, { term: 77, get: (O) => PO[O] || -1 }],
  tokenPrec: 3003
}), y = /* @__PURE__ */ v.define({
  name: "sass",
...
}), fO = /* @__PURE__ */ E((O) => O.name == "VariableName" || O.name == "SassVariableName");
function TO(O) {
  return new V(O != null && O.indented ? kO : y, y.data.of({ autocomplete: fO }));
}
export {
  TO as sass,
  fO as sassCompletionSource,
  y as sassLanguage
};
