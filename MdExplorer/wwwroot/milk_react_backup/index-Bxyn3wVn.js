import { P as M, o as $, m as E, H as j, n as y, q as S, I as G } from "./integration-DoP2a8vj.js";
var N = {};
class v {
  /**
  @internal
...
class H {
  ...
  */
  constructor(t, e) {
    this.input = t, this.ranges = e, this.chunk = "", this.chunkOff = 0, this.chunk2 = "", this.chunk2Pos = 0, this.next = -1, this.token = z, this.rangeIndex = 0, this.pos = this.chunkPos = e[0].from, this.range = e[0], this.end = e[e.length - 1].to, this.readNext();
  }
  /**
  @internal
  */
  ...
  */
  reset(t, e) {
    if (e ? (this.token = e, e.start = t, e.lookAhead = t + 1, e.value = e.extended = -1) : this.token = z, this.pos != t) {
...
function O(l, t, e, s, i, h) {
  ...
  t: for (; (n & l[r]) != 0; ) {
    ...
    for (let c = r + 3; c < a; c += 2)
      if ((l[c + 1] & n) > 0) {
        let d = l[c];
        if (o.allows(d) && (t.token.value == -1 || t.token.value == d || q(d, t.token.value, i, h))) {
          t.acceptToken(d);
          break;
        }
      }
    ...
    for (; f < p; ) {
      let c = f + p >> 1, d = a + c + (c << 1), b = l[d], L = l[d + 1] || 65536;
      if (u < b)
        p = c;
      else if (u >= L)
        f = c + 1;
...
const g = typeof process < "u" && N && /\bparse\b/.test(N.LOG);
let T = null;
function D(l, t, e) {
...
class J {
  ...
  nextFragment() {
    ...
    if (t) {
      for (this.safeFrom = t.openStart ? D(t.tree, t.from + t.offset, 1) - t.offset : t.from, this.safeTo = t.openEnd ? D(t.tree, t.to + t.offset, -1) - t.offset : t.to; this.trees.length; )
...
class Q {
  constructor(t, e, s, i) {
    this.parser = t, this.input = e, this.ranges = i, this.recovering = 0, this.nextStackID = 9812, this.minStackPos = 0, this.reused = [], this.stoppedAt = null, this.lastBigReductionStart = -1, this.lastBigReductionSize = 0, this.bigReductionCount = 0, this.stream = new H(e, i), this.tokens = new K(t, this.stream), this.topTerm = t.top[1];
    let { from: h } = i[0];
  ...
  runRecovery(t, e, s) {
    ...
    for (let r = 0; r < t.length; r++) {
      ...
      this.stream.end > n.pos ? (a == n.pos && (a++, o = 0), n.recoverByDelete(o, a), g && console.log(u + this.stackID(n) + ` (via recover-delete ${this.parser.getName(o)})`), B(n, s)) : (!i || i.score < n.score) && (i = n);
    }
    return i;
  }
...
class w extends M {
  ...
  */
  constructor(t) {
    ...
    this.nodeSet = new $(e.map((n, o) => E.define({
    ...
    }))), t.propSources && (this.nodeSet = this.nodeSet.extend(...t.propSources)), this.strict = !1, this.bufferLength = j;
    let r = x(t.tokenData);
    ...
    this.specializers = this.specializerSpecs.map(F), this.states = x(t.states, Uint32Array), this.data = x(t.stateData), this.goto = x(t.goto), this.maxTerm = t.maxTerm, this.tokenizers = t.tokenizers.map((n) => typeof n == "number" ? new m(r, n) : n), this.topRules = t.topRules, this.dialects = t.dialects || {}, this.dynamicPrecedences = t.dynamicPrecedences || null, this.tokenPrecTable = t.tokenPrec, this.termNames = t.termNames || null, this.maxNode = this.nodeSet.types.length - 1, this.dialect = this.parseDialect(), this.top = this.topRules[Object.keys(this.topRules)[0]];
  }
  createParse(t, e, s) {
