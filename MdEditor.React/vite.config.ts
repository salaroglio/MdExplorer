  ...
  return {
    ...
    closeBundle() {
      ...
      if (fs.existsSync(mainCssSource)) {
        ...
        console.log(`[vite-plugin-copy-main-css-local] Copied ${mainCssSource} to dist/css/milkdown-all.css`);
        
        // For very-simple-test.html convenience (if served from root)
        fs.copyFileSync(mainCssSource, 'milkdown-all.css');
        console.log(`[vite-plugin-copy-main-css-local] Copied ${mainCssSource} to project root/milkdown-all.css for very-simple-test.html`);
      } else {
        console.warn(`[vite-plugin-copy-main-css-local] Main CSS file ${mainCssSource} not found.`);
      }
...
function vitePluginDeployMilkdownCss() {
  ...
  return {
    ...
    closeBundle() {
      ...
      const outputAssetCssDir = 'dist/assets/css';
      ensureDirSync(outputAssetCssDir);

      // Copy individual imported CSS files
      filesToCopy.forEach(file => {
        const sourcePath = path.resolve(process.cwd(), file.src);
        const destPath = path.resolve(process.cwd(), outputAssetCssDir, file.destName);
        if (fs.existsSync(sourcePath)) {
          fs.copyFileSync(sourcePath, destPath);
          console.log(`[vite-plugin-deploy-css] Copied ${file.src} to ${outputAssetCssDir}/${file.destName}`);
        } else {
          console.warn(`[vite-plugin-deploy-css] Source CSS file not found: ${sourcePath}`);
        }
      });

      // Copy and rename milkdown-all-deploy.css to milkdown-all.css in dist/assets/css
      const deployCssSource = 'public/css/milkdown-all-deploy.css';
      const deployCssDest = path.resolve(process.cwd(), outputAssetCssDir, 'milkdown-all.css');
      if (fs.existsSync(deployCssSource)) {
        fs.copyFileSync(deployCssSource, deployCssDest);
        console.log(`[vite-plugin-deploy-css] Copied ${deployCssSource} to ${outputAssetCssDir}/milkdown-all.css`);
      } else {
        console.warn(`[vite-plugin-deploy-css] ${deployCssSource} not found. Please create and configure this file.`);
      }
