            ...
            List<Match> matches, int level, RequestInfo requestInfo)
        {
            foreach (XmlElement h1Tag in hTags)
            {
                foreach (Match itemMatch in matches.Where(_ => _.Groups[1].Value.Length == level))
                {
                    ...
                    Regex rx = new Regex(@":([^:^ ]*):", //([^:^ ]*) //replace all emoji from markdown
                              RegexOptions.Compiled | RegexOptions.IgnoreCase | RegexOptions.Multiline);
                    title = rx.Replace(title, string.Empty);
                    // replace all link

                    var content = string.Join(string.Empty, itemMatch.Groups[0].Captures);
                    // replace all emoji from html
                    ...
                    var h1InnerText = Regex.Replace(h1Tag.InnerText, EmojiPattern, "");
                    if (title.Trim() == h1InnerText.Trim())
                    {
                        // create hook
